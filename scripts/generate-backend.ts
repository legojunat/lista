...
(async function() {
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const categories = new Map<string, string>(Object.entries(categoryRows.reduce((prev, [categoryId, categoryName]) => ({
    ...prev,
  ...
  const items = new Map<string, Item>(Object.entries(itemRows.reduce((prev, [
    brickLinkPartId = '',
    name = '',
    type = '',
    categoryId = '',
  ...
  const legos = new Map<string, Lego>(Object.entries(legoRows.reduce((prev, [
    material = '',
    brickLinkPartId = '',
    brickLinkColorId = ''
  ]) => ({
  ...
  const prices = new Map<string, Price>(Object.entries(priceRows.reduce((prev, [
    material = '',
    brickLinkPartId = '',
    brickLinkColorId = '',
    minPrice = '',
  ...
  const mainGroupTops = Array.from(new Set(rows.map((row) => row.mainGroupTop)))
    .map((mainGroupTop) => {
      ...
      return {
        ...
        items: mainGroupSubs.sort().map((mainGroupSub) => {
          ...
          const categoryIds = Array.from(new Set(mainGroupSubRows
            .map(({ material }) => {
  ...
  fs.writeFileSync(path.resolve(__dirname, '../public/main-group-tops.json'), JSON.stringify(mainGroupTops, null, 2), 'utf8');
  ...
  const brickLinkCategories = categoryRows.map(([categoryId, categoryName]) => ({
    categoryId,
    categoryName,
    materials: materialsWithBrickLinkMetadata
      .filter((m) => m.categoryId === categoryId)
      .map((m) => m.material)
  }));
  fs.writeFileSync(path.resolve(__dirname, '../public/bricklink-categories.json'), JSON.stringify(brickLinkCategories, null, 2), 'utf8');
